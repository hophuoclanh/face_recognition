...
#def add_to_database(KNOWN_FACES, name):
	#KNOWN_FACES[name] = Createdata.KNOWN_FACES_ENCODINGS
...
def recognize_faces (frame):
	frame = detect.detect_face(frame)
	return frame
...
def name_authentification():
	global NAME_ADDED
	if NEW_NAME.get().lower() in Createdata.KNOWN_FACES.keys() or not len(NEW_NAME.get()):
...
Createdata.KNOWN_FACES = TrainData.recognizer
